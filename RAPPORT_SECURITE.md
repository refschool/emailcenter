...
| **Faible** | Token OAuth stocké via `pickle` | `token.pickle` est sérialisé avec `pickle`. Si un attaquant peut écrire ce fichier (cf. faille d'écriture arbitraire), un `pickle.load` malveillant exécute du code à la prochaine lecture. | Stocker le token en JSON (les credentials Google supportent `to_json`/`from_authorized_user_info`) plutôt qu'en pickle. | `auth.py` (`_save`, `get_credentials`) |
...
4. Rendre le secret webhook obligatoire ; migrer le stockage du token hors `pickle`.
